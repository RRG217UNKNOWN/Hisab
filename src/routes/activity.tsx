  ...
  const ACTIONS = useMemo(
    () =>
      [
        ...
        ["bills.updated", t("act.billUpdated")],
        ["expenses.updated", t("act.expenseUpdated")],
      ] as const,
    [t],
  );
...
function summarize(log: LogRow, tf: (k: string, vars: Record<string, string | number>) => string): string {
  ...
  if (action.startsWith("inventory_items")) {
    ...
    if (isUpdate) {
      const diff = describeDiff(
        changes as { old?: Record<string, unknown>; new?: Record<string, unknown> },
        ["stock", "price", "cost_price", "min_stock", "expiry", "name"],
      );
  ...
  if (action.startsWith("bills")) {
    if (isUpdate) {
      const newRow = (changes as { new?: Record<string, unknown> }).new ?? {};
      const invoice = getField(newRow, "invoice_number");
      const diff = describeDiff(
        changes as { old?: Record<string, unknown>; new?: Record<string, unknown> },
        ["total", "subtotal", "discount_amount", "payment_method", "invoice_date"],
      );
      return tf("act.updatedBill", { invoice }) + (diff ? ` — ${diff}` : "");
    }
  }
  if (action.startsWith("expenses")) {
    ...
    if (isUpdate) {
      const newRow = (changes as { new?: Record<string, unknown> }).new ?? {};
      const label = getField(newRow, "label");
      const diff = describeDiff(
        changes as { old?: Record<string, unknown>; new?: Record<string, unknown> },
        ["label", "amount", "cadence", "payment_method"],
      );
      return tf("act.updatedExpense", { label }) + (diff ? ` — ${diff}` : "");
    }
  }
  return action;
}
...
function describeDiff(changes: { old?: Record<string, unknown>; new?: Record<string, unknown> }, notable: string[]): string {
