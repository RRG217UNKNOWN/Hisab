        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-widest text-muted-foreground">
              <Wallet size={12} className="inline mr-1" /> shop expenses
            </p>
            <h1 className="mt-1 font-display text-3xl md:text-4xl text-ink">{t("nav.expenses")}</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              {inr(recurringMonthly)} / month recurring · {inr(oneTimeTotal)} one-time
            </p>
          </div>
          {canWrite && (
            <button
              ...
              <tbody className="divide-y divide-border">
                {scoped.map((e) => (
                  <tr key={e.id}>
                    ...
                    <td className="py-3 text-muted-foreground capitalize">{e.payment_method ?? "cash"}</td>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-ink/40 backdrop-blur-sm grid place-items-center p-4" onClick={onClose}>
      <div className="w-full max-w-md card-warm p-6 relative" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="font-display text-2xl text-ink">Add expense</h2>
        ...
        <form onSubmit={submit} className="mt-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs uppercase tracking-wider text-muted-foreground">Cadence</label>
              <select
                value={cadence}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs uppercase tracking-wider text-muted-foreground">Shop</label>
              <select
                value={shopId}
          ...
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">Party (optional)</label>
            <select
              value={partyId}
