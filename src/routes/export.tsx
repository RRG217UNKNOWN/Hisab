...
function ExportPage() {
  ...
  if (!can.viewReports(profile?.role)) {
    return (
      <AppShell active="export" title={t("impexp.exportTitle")}>
  ...
  const dataTypeLabel = (dt: DataType) =>
    dt === "inventory" ? t("impexp.dtInventory") : dt === "sales" ? t("impexp.dtSales") : dt === "expenses" ? t("impexp.dtExpenses") : t("impexp.dtBills");
  ...
  const doExport = async () => {
    ...
    if (type === "bills") q = q.eq("status", "completed");
      ...
      <div className="px-4 md:px-10 py-6 md:py-8 space-y-6 max-w-2xl">
        <header>
          <p className="text-xs uppercase tracking-widest text-muted-foreground">
            <Download size={12} className="inline mr-1" /> {t("impexp.downloadData")}
          </p>
          <h1 className="mt-1 font-display text-3xl md:text-4xl text-ink">{t("impexp.exportTitle")}</h1>
        </header>

        <div className="card-warm p-5 space-y-4">
          <div>
            <label className="block text-xs uppercase tracking-wider text-muted-foreground mb-2">{t("impexp.dataType")}</label>
            <div className="flex flex-wrap gap-2">
              {(["inventory", "sales", "expenses", "bills"] as DataType[]).map((dt) => (
                <button
                  key={dt}
                  onClick={() => setType(dt)}
                  className={`rounded-full px-4 py-1.5 text-sm capitalize transition ${
                    type === dt ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground"
                  }`}
                >
                  {dataTypeLabel(dt)}
                </button>
          ...
          <div>
            <label className="block text-xs uppercase tracking-wider text-muted-foreground mb-2">{t("exp.shop")}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="block text-xs uppercase tracking-wider text-muted-foreground mb-1">{t("impexp.from")}</span>
            ...
            <label className="block">
              <span className="block text-xs uppercase tracking-wider text-muted-foreground mb-1">{t("impexp.to")}</span>
          ...
          >
            {busy ? t("impexp.preparing") : t("impexp.download")}
          </button>
        </div>
      </div>
    </AppShell>
