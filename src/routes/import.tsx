...
function ImportPage() {
  ...
  if (!can.writeInventory(profile?.role)) {
    return (
      <AppShell active="import" title="Import">
  ...
  const handleFile = async (file: File) => {
    ...
    const parsed: RowInput[] = csv.slice(1).map((r) => {
      ...
      if (!Number.isFinite(stock) || stock < 0) errors.push("Bad stock");
      if (!Number.isFinite(price) || price < 0) errors.push("Bad price");
      return {
        name: name.trim(),
  ...
  const importAll = async () => {
    ...
    setImporting(false);
    setResult(`${added} added, ${updated} updated, ${failed} skipped`);
    setRows([]);
  };
    ...
    <AppShell active="import" title="Import">
      <div className="px-4 md:px-10 py-6 md:py-8 space-y-6 max-w-4xl">
        <header>
          ...
          <h1 className="mt-1 font-display text-3xl md:text-4xl text-ink">Import inventory</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            CSV columns: <code>name, category, stock, min_stock, price, cost_price, expiry</code>
          </p>
        </header>

        <div className="card-warm p-5 space-y-3">
          <label className="block">
            <span className="block text-xs uppercase tracking-wider text-muted-foreground mb-1">Target shop</span>
            <select
              value={shopId}
          ...
          <label className="block border-2 border-dashed border-border rounded-lg p-8 text-center cursor-pointer hover:bg-muted/30">
            ...
            <div className="mt-2 text-sm text-ink">Choose CSV file</div>
            <input
              type="file"
              accept=".csv,text/csv"
        ...
        {rows.length > 0 && (
          <div className="card-warm p-4 md:p-6">
            <div className="flex items-baseline justify-between mb-3">
              <h2 className="font-display text-xl text-ink">Preview — {rows.length} rows</h2>
              <button
                onClick={importAll}
                ...
                <tbody className="divide-y divide-border">
                  {rows.map((r, i) => (
                    <tr key={i} className={r.errors.length ? "bg-destructive/5" : ""}>
                      ...
                      <td className="py-2 text-right tabular-nums">₹{r.price}</td>
                      <td className="py-2 text-xs text-muted-foreground">{r.matchId ? "Add to existing" : "Create new"}</td>
