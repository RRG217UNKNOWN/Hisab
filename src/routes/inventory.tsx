...
import { ExpenseItemsManager, type ExpenseItem } from "@/components/ExpenseItemsManager";
...
function InventoryPage() {
  ...
  const canManageChart = can.writeExpenses(profile?.role);
  ...
  const loadExpenseItems = useCallback(async () => {
    ...
    if (e) return;
    setExpenseItems((data ?? []).map((i) => ({ ...i, usage_count: Number(i.usage_count) })));
  }, [user]);

  useEffect(() => {
    if (user) void loadExpenseItems();
  }, [user, loadExpenseItems]);
    ...
    <AppShell active="inventory" title={t("inv.title")}>
      ...
      {chartOpen && user && (
        <ExpenseItemsManager
          ...
          onChanged={() => void loadExpenseItems()}
        />
      )}
    </AppShell>
  );
