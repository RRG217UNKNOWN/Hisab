...
function PartyDetail() {
  ...
  const typeLabel = (ty?: PartyRow["type"]) =>
    ty === "vendor" ? t("party.vendor") : ty === "customer" ? t("party.customer") : ty === "both" ? t("party.both") : "";

  return (
    <AppShell active="parties" title={party?.name ?? t("party.title")}>
      <div className="px-4 md:px-10 py-6 md:py-8 space-y-6 max-w-4xl">
        <Link to="/parties" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft size={14} /> {t("party.allParties")}
        </Link>
        ...
        {party && (
          <header>
            <p className="text-xs uppercase tracking-widest text-muted-foreground">{typeLabel(party.type)}</p>
        ...
        <section className="grid gap-3 sm:grid-cols-3">
          <StatCard label={t("party.youOwe")} value={inr(Math.max(0, totals.payable))} tone="danger" />
          <StatCard label={t("party.owesYou")} value={inr(Math.max(0, totals.receivable))} tone="success" />
          <StatCard
            label={t("party.netBalance")}
        ...
        <div className="flex justify-end gap-2">
          ...
          {canPay && (
            ...
            >
              <Plus size={14} /> {t("party.recordPayment")}
            </button>
          )}
        </div>

        <section className="card-warm p-4 md:p-6">
          <h2 className="font-display text-xl text-ink mb-3">{t("party.transactionHistory")}</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wider text-muted-foreground">
                  <th className="py-2 font-medium">{t("party.date")}</th>
                  <th className="py-2 font-medium">{t("party.description")}</th>
                  <th className="py-2 font-medium">{t("party.type")}</th>
                  <th className="py-2 font-medium text-right">{t("party.amount")}</th>
                </tr>
              </thead>
...
function PaymentModal({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!Number.isFinite(n) || n <= 0) {
      setErr(t("party.enterPositiveAmount"));
      return;
    }
    setBusy(true);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-ink/40 backdrop-blur-sm grid place-items-center p-4" onClick={onClose}>
      <div className="w-full max-w-md card-warm p-6 relative" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} aria-label={t("common.close")} className="absolute right-3 top-3 h-8 w-8 grid place-items-center rounded-full hover:bg-muted">
          <X size={16} />
        </button>
        <h2 className="font-display text-2xl text-ink">{t("party.recordPayment")}</h2>
        <form onSubmit={submit} className="mt-5 space-y-3">
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">{t("party.direction")}</label>
            <select
              value={direction}
              onChange={(e) => setDirection(e.target.value as "payable" | "receivable")}
              className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm"
            >
              <option value="payable">{t("party.paidThem")}</option>
              <option value="receivable">{t("party.theyPaidMe")}</option>
            </select>
          </div>
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">{t("party.amount")} (₹)</label>
            <input
              type="number"
              value={amount}
          ...
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">{t("party.note")}</label>
            <input
              value={note}
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={busy} className="rounded-lg bg-primary text-primary-foreground px-4 py-2 text-sm disabled:opacity-60">
              {busy ? t("party.saving") : t("party.record")}
            </button>
          </div>
        </form>
      </div>
