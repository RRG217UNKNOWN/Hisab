...
import { Plus, Search, Contact, TrendingUp, TrendingDown } from "lucide-react";
...
function PartiesPage() {
  ...
  return (
    <AppShell active="parties" title="Parties">
      <div className="px-4 md:px-10 py-6 md:py-8 space-y-6">
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-widest text-muted-foreground">
              <Contact size={12} className="inline mr-1" /> customers & vendors
            </p>
            <h1 className="mt-1 font-display text-3xl md:text-4xl text-ink">Parties</h1>
            <p className="mt-1 text-sm text-muted-foreground">{parties.length} parties</p>
          </div>
          {canManage && (
            <button
        ...
        <section className="grid gap-3 sm:grid-cols-2">
          <div className="card-warm p-5">
            <div className="text-xs uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
              <TrendingDown size={13} className="text-destructive" /> You owe
            </div>
            <div className="mt-2 font-display text-3xl text-ink tabular-nums">{inr(orgTotals.payable)}</div>
          </div>
          <div className="card-warm p-5">
            <div className="text-xs uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
              <TrendingUp size={13} className="text-success" /> Owed to you
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wider text-muted-foreground">
                  ...
                  <th className="py-2 font-medium text-right">You owe</th>
                  <th className="py-2 font-medium text-right">Owed to you</th>
              ...
              <tbody className="divide-y divide-border">
                {filtered.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-muted/40 cursor-pointer">
                      <td className="py-3">
                        <Link to="/parties/$id" params={{ id: p.id }} className="text-ink hover:underline">
                          {p.name}
                        </Link>
                      </td>
                      <td className="py-3 text-muted-foreground text-xs capitalize">{p.type}</td>
