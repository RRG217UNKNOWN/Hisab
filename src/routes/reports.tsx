...
import { ClassifyTransactionModal, type ClassificationRow, type TxnCandidate } from "@/components/ClassifyTransactionModal";
...
function ReportsBody({
  ...
  const load = useCallback(async () => {
    if (!userId) return;
    const [wh, ms, sl, ex, bl, pa, lg, cl] = await Promise.all([
      ...
      sb.from("bills").select("*").eq("status", "completed").order("invoice_date", { ascending: false }),
      sb.from("parties").select("id, name"),
      sb.from("party_ledger").select("party_id, date, direction, amount"),
      sb.from("transaction_classifications").select("*").order("created_at", { ascending: false }),
    ]);
    ...
    else if (cl.error) setErr(cl.error.message);
    else setErr(null);
    setClassifications((cl.data as ClassificationRow[] | null) ?? []);
  ...
  // Reuses the exact same "Add Party" gate — anyone who can manage parties
  // can tag/categorize a transaction here too.
  const canClassify = can.manageParties(role);

  // Candidates offered in the classification picker: every bill (by
  // invoice # or party) and every expense (by label), most recent first.
  const txnCandidates = useMemo<TxnCandidate[]>(() => {
    const billCandidates: TxnCandidate[] = bills.map((b) => ({
      label: `${b.invoice_number || b.id.slice(0, 8)} · ${b.bill_type === "sale" ? b.customer_name : b.supplier_name || "—"}`,
      source_type: "bill",
      source_id: b.id,
    }));
    const expenseCandidates: TxnCandidate[] = expenses.map((e) => ({
      label: e.label,
      source_type: "expense",
      source_id: e.id,
    }));
    return [...billCandidates, ...expenseCandidates];
  }, [bills, expenses]);

  const classificationsByBill = useMemo(() => {
    const map = new Map<string, ClassificationRow>();
    for (const c of classifications) {
      if (c.source_type === "bill" && c.source_id) map.set(c.source_id, c);
    }
    return map;
  }, [classifications]);
    ...
    <AppShell active="reports" title={t("rep.title")}>
      <div className="px-4 md:px-10 py-6 md:py-8 space-y-6">
        <header className="flex flex-wrap items-end justify-between gap-4">
          ...
          {canClassify && (
            ...
            >
              <Tag size={14} /> {t("rep.classifyTransaction")}
            </button>
          )}
        </header>
        ...
        {classifications.length > 0 && (
          <ClassificationsPanel
            ...
            onDeleted={(id) => setClassifications((prev) => prev.filter((c) => c.id !== id))}
          />
        )}
        ...
        {tab === "bills" && (
          <BillsReport bills={scopedBills} partyName={partyName} warehouses={warehouses} classificationsByBill={classificationsByBill} />
        )}
      ...
      {classifyOpen && userId && (
        <ClassifyTransactionModal
          userId={userId}
          candidates={txnCandidates}
          onClose={() => setClassifyOpen(false)}
          onSaved={(row) => {
            setClassifications((prev) => [row, ...prev]);
...
function ClassificationsPanel({
  ...
  const remove = async (id: string) => {
    ...
    const { error } = await sb.from("transaction_classifications").delete().eq("id", id);
  ...
  return (
    <section className="card-warm p-4 md:p-5">
      <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">
        <Tag size={14} /> Recent classifications
      </div>
        ...
        {classifications.slice(0, 5).map((c) => (
          <div key={c.id} className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2">
            <div className="min-w-0">
              <div className="text-sm text-ink truncate">{c.reference_label}</div>
              {c.notes && <div className="text-xs text-muted-foreground truncate">{c.notes}</div>}
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${typeStyle(c.classification)}`}>{c.classification}</span>
              {canManage && (
                <button
                  onClick={() => remove(c.id)}
...
function BillsReport({
  ...
  classificationsByBill,
}: {
  bills: Bill[];
  partyName: (id: string | null) => string;
  warehouses: Warehouse[];
  typeFilterDefault?: "all" | "sale" | "purchase";
  classificationsByBill?: Map<string, ClassificationRow>;
}) {
                ...
                <tr key={b.id} className="cursor-pointer hover:bg-muted/40" onClick={() => setOpenBillId(b.id)}>
                  ...
                  <td className="py-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      {classificationsByBill?.get(b.id) && (
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
                          {classificationsByBill.get(b.id)!.classification}
                        </span>
                      )}
                    </div>
                  </td>
        ...
        {bill ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2 print:hidden">
              ...
              <ShareBillMenu bill={bill} seller={seller} className="rounded-full border border-border bg-card px-4 py-2 text-sm inline-flex items-center gap-1.5" />
