...
type ConnRow = { id: string; org_id: string; partner_org_id: string; status: "pending" | "connected" };
...
function RequestsPage() {
  ...
  const [side, setSide] = useState<"sent" | "received" | "connections">("received");
  ...
  const load = useCallback(async () => {
    ...
    const [req, po, wh, ms] = await Promise.all([
      ...
      sb.from("partner_orgs").select("id, org_id, partner_org_id, status"),
    ...
    const allConns = ((po.data as ConnRow[] | null) ?? []).filter((c) => c.org_id === myOrg || c.partner_org_id === myOrg);
    setConnRows(allConns);
    const connected = allConns.filter((c) => c.status === "connected");
    const partnerIds = connected.map((c) => (c.org_id === myOrg ? c.partner_org_id : c.org_id));
    const orgIds = Array.from(
      new Set([...allConns.flatMap((c) => [c.org_id, c.partner_org_id]), ...rows.flatMap((r) => [r.from_org_id, r.to_org_id])]),
    );
  ...
  const connectedList = useMemo(() => connRows.filter((c) => c.status === "connected"), [connRows]);
  const incomingConnRequests = useMemo(
    () => connRows.filter((c) => c.status === "pending" && c.partner_org_id === myOrg),
    [connRows, myOrg],
  );
  const outgoingConnRequests = useMemo(
    () => connRows.filter((c) => c.status === "pending" && c.org_id === myOrg),
    [connRows, myOrg],
  );
  const connCounterpartName = useCallback((c: ConnRow) => orgNames[c.org_id === myOrg ? c.partner_org_id : c.org_id] ?? "—", [orgNames, myOrg]);

  const acceptConnection = async (c: ConnRow) => {
    const { error } = await sb.from("partner_orgs").update({ status: "connected" }).eq("id", c.id);
    if (error) setErr(error.message);
    else void load();
  };
  const removeConnection = async (c: ConnRow) => {
    if (!confirm("Remove this connection?")) return;
    const { error } = await sb.from("partner_orgs").delete().eq("id", c.id);
    if (error) setErr(error.message);
    else void load();
  };
      ...
      <div className="px-4 md:px-10 py-6 md:py-8 max-w-3xl space-y-6">
        ...
        {!myOrg && (
          <div className="card-warm p-6 text-sm text-muted-foreground">
            Requests work between organisations — join or create an org first (see Settings).
          </div>
        )}
        ...
        {myOrg && (
          <>
            <div className="inline-flex items-center gap-1 rounded-full border border-border bg-card p-0.5 text-sm">
              ...
              {canSend && (
                <button
                  onClick={() => setSide("connections")}
                  className={`px-4 py-1.5 rounded-full transition ${side === "connections" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
                >
                  Connections
                </button>
              )}
            ...
            {side === "connections" ? (
              <div className="space-y-3">
                <ConnSection title={`Connected (${connectedList.length})`}>
                  {connectedList.map((c) => (
                    <ConnCard key={c.id} name={connCounterpartName(c)} status="connected" onRemove={() => removeConnection(c)} />
                  ))}
                  {connectedList.length === 0 && <ConnEmpty>No connections yet.</ConnEmpty>}
                </ConnSection>
                <ConnSection title={`Incoming requests (${incomingConnRequests.length})`}>
                  {incomingConnRequests.map((c) => (
                    <ConnCard
                      key={c.id}
                      name={connCounterpartName(c)}
                      status="pending"
                      onAccept={() => acceptConnection(c)}
                      onRemove={() => removeConnection(c)}
                    />
                  ))}
                  {incomingConnRequests.length === 0 && <ConnEmpty>None.</ConnEmpty>}
                </ConnSection>
                <ConnSection title={`Sent, waiting (${outgoingConnRequests.length})`}>
                  {outgoingConnRequests.map((c) => (
                    <ConnCard key={c.id} name={connCounterpartName(c)} status="pending" onRemove={() => removeConnection(c)} />
                  ))}
                  {outgoingConnRequests.length === 0 && <ConnEmpty>None.</ConnEmpty>}
                </ConnSection>
              </div>
            ) : (
              <>
...
function ConnSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
...
function ConnEmpty({ children }: { children: React.ReactNode }) {
